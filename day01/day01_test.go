...
func Test_solveForSeries(t *testing.T) {
	type args struct {
		s []int
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := solveForSeries(tt.args.s)
			if (err != nil) != tt.wantErr {
				t.Errorf("solveForSeries() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != tt.want {
				t.Errorf("solveForSeries() = %v, want %v", got, tt.want)
			}
		})
	}
}
func Benchmark_solveForSeries(b *testing.B) {
	type args struct {
		s []int
	}
	benchmark := struct {
	...
	b.Run(benchmark.name, func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, err := solveForSeries(benchmark.args.s)
			if err != nil {
				b.FailNow()
			}
		}
